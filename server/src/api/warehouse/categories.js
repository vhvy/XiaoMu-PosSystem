...
route.post("/create", validBody(
    ...
    if (parent_name) {
        const validParentCategoryResult = await CategoriesTask.getCategoryDetails(parent_name);
...
route.post("/updateparent", validBody(
    ...
    const validParentNameResult = await CategoriesTask.getCategoryDetails(parent_name);
